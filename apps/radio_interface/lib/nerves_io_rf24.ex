...
#        Nerves.IO.Supervisor.start_link(name: Nerves.IO.Supervisor)
#  end 
  ...
  The following options are available:
    ...
    * `:data_rate` - (0,2) RF24_1MBPS (0), RF24_2MBPS (1), RF24_250KBPS (2)
...
#  end
 ...
 @env Mix.env
  ...
  def handle_call({:open, name, opts}, {from_pid, _}, state) do
    ...
    IO.puts "Step 20.1"
    #IO.inspect response
  # data_rate=elem(response, 1)[:data_rate]
   #IO.inspect data_rate
    case(response) do
	{:exit_status, :ok} ->
            exit({:exit_status, "radio not available"})          
	_->  
           IO.puts "radio set"
           new_state =%{
              state
                  | name: name,
                    ...
                    is_active: is_active,
	            data_rate: elem(response, 1)[:data_rate],
                    CRC_length: elem(response, 1)[:CRC_length],
                    PA_level: elem(response, 1)[:PA_level],
                    reading_pipe: elem(response, 1)[:reading_pipe],
                    writing_pipe: elem(response, 1)[:writing_pipe],
                    RFchannel: elem(response, 1)[:RFchannel]
            }
            {:reply, Map.fetch(state, :port), new_state}
	end
# Map.fetch(elem(response, 2), :data_rate)
#    new_state =
#      change_framing(
#        %{
#          state
...
#  end
   ...
   def handle_info({port, {:data, data}}, state = %State{port: port}) do
    ...
    def handle_info({port, {:config, data}},state = %State{port: port}) do
      IO.puts "step14A"
      Logger.info "Exit code #{inspect data}"
#      cleanup(reason, state)
       {:stop, data, state} # see GenServer docs for other return types
    end

    # handle the trapped exit call
    def handle_info({:exit_status, _from, reason}, state) do
      Logger.info "Exit code #{inspect reason}"
#      cleanup(reason, state)
 ...
 def handle_info({:port_timed_out,ref}, state) do
     #Port.flush(port)
     IO.puts "port timed out"
     Logger.info "Port timed out #{inspect ref}"
  end
...
#  end
  ...
  defp call_port(state, command, arguments, timeout \\ 4000) do
    ...
    after
       ...
       # exit({:port_timed_out, command})
       {:port_timed_out,command}
   end
end
